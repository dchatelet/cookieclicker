...
function FireEvent(el, ev) {
	if (el.fireEvent) { el.fireEvent('on' + ev); }
	else {
...
Game.Launch = function () {
	...
	Game.Init = function () {
		...
		Game.showLangSelection = function () {
			...
			for (var i in Langs) {
				var lang = Langs[i];
				AddEvent(l('langSelect-' + i), 'click', function (lang) {
					return function () {
						if (lang != locId) {
							PlaySound('snd/tick.mp3');
		...
		var getStrThousandFingersGain = function (x) { return loc("Multiplies the gain from %1 by <b>%2</b>.", [getUpgradeName("Thousand fingers"), x]); }
